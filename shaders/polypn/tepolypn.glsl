...
vec3 pnTessellate(vec3 p) {
    vec3 sum = vec3(0.0, 0.0, 0.0);
    vec3 sum2;
    float weightproduct = 1.0;
    vec3 pointsum = vec3(0.0, 0.0, 0.0);
    float factor= 0.0;
    for(int i = 0; i < N; i++) {
        sum2 = vec3(0.0, 0.0, 0.0);
        for(int j = 0; j < N; j++) {
            if(i != j) {
                sum2 += weights[j] * p;
            } else {
                sum2 += weights[j] * project(normalize(tc_normal[j]), tc_position[j], p);
            }
        }
        sum += weights[i] * sum2;
        weightproduct *= weights[i];
        pointsum += tc_position[i];
        factor += 1.0;
    }
    ...
    for(int i = 0; i < N-2; i++) {
        for(int j = i+1; j < N-1; j++) {
            for(int k = j+1; k < N; k++) {
                vertexsum = tc_position[i] + tc_position[j] + tc_position[k];
                sum2 = -3.0 * vertexsum;

                sum2 += 3.0*project(normalize(tc_normal[i]), tc_position[i], vertexsum/3.0);
                sum2 += 3.0*project(normalize(tc_normal[j]), tc_position[j], vertexsum/3.0);
                sum2 += 3.0*project(normalize(tc_normal[k]), tc_position[k], vertexsum/3.0);

                weightproduct = weights[i] * weights[j] * weights[k];
                sum += 6.0*weightproduct*(sum2/12.0);
            }
