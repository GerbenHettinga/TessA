...
void main()
{
    ...
    for(int j = 0; j < polygonSize; j++) {
        if(gl_PrimitiveIDIn != j) {
            ...
            flatPos = (ps[gl_PrimitiveIDIn] + 2.0*ps[j % polygonSize])/3.0;
            patchPos = (project(ns[j % polygonSize], ps[j % polygonSize], ps[gl_PrimitiveIDIn]) + 2.0*ps[j % polygonSize])/3.0;
            position = mix(flatPos, patchPos, alpha);
...
//    EmitVertex();
    ...
    patchPos = (project(ns[p1], ps[p1], centre) + 2.0*ps[p1])/3.0;
    position = mix(flatPos, patchPos, alpha);
    normal = vec3(0.0, 1.0, 0.0);
    gl_Position = matrix * vec4(position, 1.0);
    EmitVertex();
