...
vec4 findRatios(vec3 F0, vec3 F3,
                float H0, vec3 H0Hat, float H2, vec3 H2Hat, vec3 C0, vec3 C3) {
    float k0, h0, k1, h1;
...
void main()
{
    ...
    int p3 = (gl_InvocationID + 3) % 4;
    int p2 = (gl_InvocationID + 2) % 4;
    int p1 = (gl_InvocationID + 1) % 4;
    ...
    vec3 H1 = cps[p1].b[2] - cps[gl_InvocationID].b[0];
    vec3 H2 = position[p1] - cps[p1].b[2];
    ...
    cps[gl_InvocationID].f[1] = cps[p1].b[2] + k1*C2 - (k1-k0)*C3/3.0 + h0*H2/3.0 + 2.0*h1*H1/3.0;
    ...
    cps[gl_InvocationID].f[3] = cps[p1].b[2] + k1*C2 - (k1-k0)*C3/3.0 + h0*H2/3.0 + 2.0*h1*H1/3.0;
