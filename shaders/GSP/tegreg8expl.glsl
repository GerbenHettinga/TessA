...
vec3 evaluatePatchCubWD() {
    ...
    for(int i = 0; i < 8; i++) {
        ...
        point += 3.0 * sqd[i] * sqd[(i+6) % N] * cps[i].b[5];
        ...
        point += 12.0 *  (weights[wrapper(i-1, N)] * sqd[i] * weights[(i + 1) % N]) *
                ...
                (1.0-weights[(i + 1) % N]) * weights[wrapper(i-1, N)] * cps[wrapper(i-1, N)].f[11])
                /
                ((1.0-weights[wrapper(i-1, N)]) * weights[(i + 1) % N] + (1.0-weights[(i + 1) % N]) * weights[wrapper(i-1, N)]);

        sumW[i] += 12.0 * (weights[wrapper(i-1, N)] * sqd[i] * weights[(i + 1) % N] +
                    weights[wrapper(i-1, N)] * weights[i] * sqd[(i+1) % N] +
                    sqd[i] * weights[(i+1) % N] * weights[(i+6) % N] +
                    weights[i] * sqd[(i+1) % N] * weights[(i+6) % N] +
                    sqd[i] * weights[(i+1) % N] * weights[(i+5) % N] +
                    weights[i] * sqd[(i+1) % N] * weights[(i+5) % N] +
                    sqd[i] * weights[(i+1) % N] * weights[(i+4) % N] +
                    weights[i] * sqd[(i+1) % N] * weights[(i+4) % N] +
                    sqd[i] * weights[(i+1) % N] * weights[(i+3) % N] +
                    weights[i] * sqd[(i+1) % N] * weights[(i+3) % N] +
                    sqd[i] * weights[(i+1) % N] * weights[(i+2) % N]);


       point += 12.0 *(
            weights[wrapper(i-1, N)] * weights[i] * sqd[(i+1) % N] * cps[i].f[1] +

            sqd[i] * weights[(i+1) % N] * weights[(i+6) % N] * cps[i].f[2] +
            weights[i] * sqd[(i+1) % N] * weights[(i+6) % N] * cps[i].f[3] +

            sqd[i] * weights[(i+1) % N] * weights[(i+5) % N] * cps[i].f[4] +
            weights[i] * sqd[(i+1) % N] * weights[(i+5) % N] * cps[i].f[5] +

            sqd[i] * weights[(i+1) % N] * weights[(i+4) % N] * cps[i].f[6] +
            weights[i] * sqd[(i+1) % N] * weights[(i+4) % N] * cps[i].f[7] +

            sqd[i] * weights[(i+1) % N] * weights[(i+3) % N] * cps[i].f[8] +
            weights[i] * sqd[(i+1) % N] * weights[(i+3) % N] * cps[i].f[9] +

            sqd[i] * weights[(i+1) % N] * weights[(i+2) % N] * cps[i].f[10]);
    }

    float s = sumW[0];
    vec3 C = 0.5 * (cps[0].f[5] + cps[0].f[6]);
    for(int i = 1; i < 8; i++) {
        s += sumW[i];
        C += 0.5 * (cps[i].f[5] + cps[i].f[6]);
    }

    point += 0.125 * (1.0 - s) * (C + (C - flatPoint) * qvalue);
...
vec3 evaluatePatchCub() {
    ...
    for(int i = 0; i < N; i++) {
       ...
       point += 12.0 *(
        ...
        //free points
            weights[i] * weights[(i+1) % N] * sqd[(i+6) % N] * cps[(i + 6) % N].f[5] +
            weights[i] * weights[(i+1) % N] * sqd[(i+5) % N] * cps[(i + 4) % N].f[5] +
            weights[i] * weights[(i+1) % N] * sqd[(i+4) % N] * cps[(i + 4) % N].f[2] +
            weights[i] * weights[(i+1) % N] * sqd[(i+3) % N] * cps[(i + 3) % N].f[6] +

            sqd[i] * weights[(i+6) % N] * weights[(i+3) % N] * cps[(i + 6) % N].f[6] +
            weights[i] * sqd[(i+6) % N] * weights[(i+3) % N] * cps[(i + 6) % N].f[6] +
            weights[i] * weights[(i+6) % N] * sqd[(i+3) % N] * cps[(i + 6) % N].f[6] +

            sqd[i] * weights[(i+6) % N] * weights[(i+4) % N] * cps[(i + 6) % N].f[6] +
            weights[i] * sqd[(i+6) % N] * weights[(i+4) % N] * cps[(i + 6) % N].f[6] +
            weights[i] * weights[(i+6) % N] * sqd[(i+4) % N] * cps[(i + 6) % N].f[6]);
        ...
        point += 12.0 * weights[i] * weights[(i + 1) % N] * weights[(i + 2) % N] * weights[(i + 5) % N] * (cps[i % N].f[2] + cps[(i + 1) % N].f[5]);
...
vec3 evaluateBoundaryCurve() {
    ...
    return weights[side] * weights[side] * weights[side] * tc_position[side] +
            ...
            3.0 * weights[p1] * weights[side] * weights[p1] * cps[p1].b[6] +
