...
vec3 evaluatePatchCub() {
    ...
    for(int i = 0; i < N; i++) {
        ...
        point += 12.0 *( (weights[wrapper(i-1, N)] * sqd[i] * weights[(i + 1) % N]) *
                ...
                (1.0-weights[(i + 1) % N]) * weights[wrapper(i-1, N)] * cps[wrapper(i-1, N)].f[7])
                /
                ((1.0-weights[wrapper(i-1, N)]) * weights[(i+1) % N] + (1.0-weights[(i+1) % N]) * weights[wrapper(i-1, N)]));

        point += 12.0 * weights[wrapper(i-1, N)] * weights[i] * sqd[(i + 1) % N] * cps[i].f[1];

        point += 12.0 * sqd[i] * weights[(i+1) % N] * weights[(i+4) % N] * cps[i].f[2];
        point += 12.0 * weights[i] * sqd[(i+1) % N] * weights[(i+4) % N] * cps[i].f[3];

        point += 12.0 * sqd[i] * weights[(i+1) % N] * weights[(i+3) % N] * cps[i].f[4];
        point += 12.0 * weights[i] * sqd[(i+1) % N] * weights[(i+3) % N] * cps[i].f[5];

        point += 12.0 * sqd[i] * weights[(i+1) % N] * weights[(i+2) % N] * cps[i].f[6];

        //free points
        point += 12.0 * weights[i] * weights[(i+1) % N] * sqd[(i+4) % N] * cps[(i + 3) % N].f[5];
        point += 12.0 * weights[i] * weights[(i+1) % N] * sqd[(i+3) % N] * cps[(i + 2) % N].f[2];

        point += 12.0 * sqd[i] * weights[(i+2) % N] * weights[(i+4) % N] * cps[i].f[3];
...
vec3 evaluateBoundaryCurve() {
    ...
    return weights[side] * weights[side] * weights[side] * tc_position[side] +
            ...
            3.0 * weights[p1] * weights[side] * weights[p1] * cps[p1].b[4] +
...
void main() {
    ...
    outColour = vec3((6 % 3)/8.0 + 0.5, (6 % 2)/8.0 + 0.5, (6 % 5)/8.0 + 0.5);
    if(outline && (weights[0] < 0.0001 || weights[1] < 0.0001 || weights[2] < 0.0001 || weights[3] < 0.0001 || weights[4] < 0.0001 || weights[5] < 0.0001)) {
        outColour = vec3(0.0);
    }
