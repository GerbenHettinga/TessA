#include "gbsrenderer.h"
#include <iostream>

GBSRenderer::GBSRenderer(RenderParameters* rp)
...
void GBSRenderer::initBuffers() {
    ...
    for(GLuint i = 0; i < 9; i++) {
        ...
        glVertexAttribPointer(i, 3, GL_FLOAT, GL_FALSE, 9*sizeof(glm::vec3), (void*) (i*sizeof(glm::vec3)));
    }
    glBindVertexArray(0);
...
void GBSRenderer::initShaders() {
    //qDebug() << "GBS shaders EV";
    ...
    for(size_t i = 0; i < 7; ++i) {
        polyShaders.push_back(new PolygonShader(i + 3,
                                             std::string("../shaders/gbs/cubic/vertshadergbsgb.glsl"),
                                             std::string("../shaders/gbs/cubic/EV/tcgbsgbx.glsl"),
                                             std::string("../shaders/gbs/cubic/EV/tegbsgbx.glsl") ));
    ...
    //qDebug() << "GBS shaders EF";

    for(size_t i = 0; i < 7; ++i) {
        polyShaders.push_back(new PolygonShader(i + 3,
                                             std::string("../shaders/gbs/cubic/vertshadergbsgb.glsl"),
                                             std::string("../shaders/gbs/cubic/EF/tcgbsgbx.glsl"),
                                             std::string("../shaders/gbs/cubic/EF/tegbsgbx.glsl") ));
...
void GBSRenderer::updateBuffers(Model& model) {
...
void GBSRenderer::paintTessellation(int valency, int indexBufPointer, int numPatches, bool EV) {
...
void GBSRenderer::render(Model& model) {
