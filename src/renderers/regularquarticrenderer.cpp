...
RegularQuarticRenderer::RegularQuarticRenderer(RenderParameters* rp)
    : SurfaceRenderer(rp)
{
    getError();
    initBuffers();
...
RegularQuarticRenderer::~RegularQuarticRenderer() {

}

void RegularQuarticRenderer::initBuffers() {
...
void RegularQuarticRenderer::initShaders() {
    polyShaders.push_back(new PolygonShader(4,
                                         std::string("../shaders/quarticacc1/vertshaderqacc1.glsl"),
                                         std::string("../shaders/quarticacc1/tcqacc1.glsl"),
                                         std::string("../shaders/quarticacc1/teqacc1.glsl") ));
...
void RegularQuarticRenderer::updateBuffers(Model& model) {
...
void RegularQuarticRenderer::render(Model& model) {
    glBindVertexArray(vao);

    GenQuarticBSplineMesh& m = model.getQuarticGBSMesh();

    if(!m.getRegularBs().empty()) {
        ...
        int numPatches = m.getRegularBs().size() / 9;
